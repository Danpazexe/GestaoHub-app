...
import { View, Text, Switch, TouchableOpacity, StyleSheet, ScrollView, Dimensions } from 'react-native';
...
const NotificationScreen = ({ navigation, isDarkMode }) => {
  ...
  const [alertLevel] = useState('normal');
  ...
  const loadNotificationStats = async () => {
    try {
      setNotificationStats(await getNotificationStats());
    } catch (error) {
  ...
  const clearPastNotifications = async () => {
    try {
      setIsLoading(true);
      const nextStats = await clearScheduledNotifications();
  ...
  const testNotification = async () => {
    try {
      await sendTestNotification({ alertLevel, vibrationEnabled });
  ...
  const updateNotificationSchedule = async () => {
    ...
    try {
      setIsLoading(true);
      const result = await scheduleProductExpiryNotifications({ vibrationEnabled });
      ...
      if (result.scheduledCount > 0) {
        Toast.show({
          ...
          text2: `${result.scheduledCount} notificação(ões) agendadas com sucesso!\n\nEstratégia automática:\n• 1-7 dias: Notificação diária (CRÍTICO)\n• 8-14 dias: Dia sim, dia não (ATENÇÃO)\n• 15-30 dias: Uma vez só (NORMAL)`
        });
      } else {
        Toast.show({
          type: 'info',
          text1: 'Nenhuma Notificação',
          text2: 'Nenhum produto encontrado com data de vencimento nos próximos 30 dias para agendar notificações.'
        });
      }

      if (result.scheduledCount >= 450) {
        Toast.show({
          type: 'error',
      ...
      >
        ...
        <View style={[styles.infoContainer, isDarkMode ? styles.darkInfoContainer : styles.lightInfoContainer]}>
          ...
          <Text style={[styles.infoText, isDarkMode ? styles.darkText : styles.lightText]}>
            ...
            • 15-30 dias restantes: Uma notificação ÚNICA (NORMAL){'\n'}
