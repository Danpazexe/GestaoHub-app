...
import { CORESEXCEL } from '../../../shared/components/coresAuth';
...
const ExcelScreen = ({ navigation, isDarkMode }) => {
  ...
  const loadHistory = useCallback(async () => {
    try {
      const exportDate = await AsyncStorage.getItem('lastExportDate');
      const importDate = await AsyncStorage.getItem('lastImportDate');
  ...
  const exportToExcel = async () => {
    ...
    try {
      ...
      await AsyncStorage.setItem('lastExportDate', now);
      setLastExport(now);
  ...
  const importFromExcel = async () => {
    ...
    try {
      ...
      const now = new Date().toISOString();
      await AsyncStorage.setItem('lastImportDate', now);
      setLastImport(now);
