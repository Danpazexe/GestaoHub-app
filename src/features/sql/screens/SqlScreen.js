...
import { clearSqlCacheProducts, replaceSqlCacheProducts } from '../services/sqlCacheService';
import { sqlTheme } from '../../../theme/domains/sql';

const PRIMARY_COLOR = sqlTheme.primary;
...
const SqlScreen = ({ isDarkMode, navigation }) => {
  ...
  const onRefresh = React.useCallback(async () => {
    ...
    try {
      ...
      const parsedCache = await loadCachedProducts();
      ...
      try {
        ...
        if (produtos.length > 0) {
          ...
          if (isValidStructure) {
            ...
            try {
              await replaceSqlCacheProducts(normalizedData);

              // Verifica se o dado realmente ficou persistido (evita "sumir" ao sair da tela)
              const persisted = await loadCachedProducts();
              const persistedCount = Array.isArray(persisted) ? persisted.length : 0;
  ...
  const cacheData = async (data) => {
    try {
      await replaceSqlCacheProducts(data);
    } catch (error) {
  ...
  const loadCachedData = useCallback(async () => {
    try {
      const parsedData = await loadCachedProducts();
