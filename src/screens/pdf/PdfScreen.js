...
const PdfScreen = ({ isDarkMode }) => {
  ...
  const compartilharPDF = async (mostrarLegendaAtual) => {
    ...
    try {
      ...
      let produtos = productsJson ? JSON.parse(productsJson) : [];
      ...
      produtos = produtos.sort((a, b) => {
        if (orderBy === 'validade') {
          ...
          return orderDirection === 'asc' ? (a.quantidade - b.quantidade) : (b.quantidade - a.quantidade);
        }
        return 0;
      });
      ...
      let htmlTemplate = await loadHtmlTemplate();
      if (!htmlTemplate) {
        throw new Error('Não foi possível carregar o template HTML');
      }
        ...
        <View style={{ padding: 18 }}>
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              setLoading(true);
              try {
                // Gera o PDF, mas não compartilha, apenas retorna o caminho
                const productsJson = await AsyncStorage.getItem('products');
                let produtos = productsJson ? JSON.parse(productsJson) : [];
