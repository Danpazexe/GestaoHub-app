...
import { CORESCONFERENCIASAIDA } from '../../../components/coresAuth';
import { STORAGE_KEYS } from '../../../constants/storage';
import { computeProgress, computeTotals, normalizeKey, pluralize } from '../shared/conferenciaCore';
import { buildExpectedItemsSaida } from '../shared/conferenciaMock';
...
const ConferenciaSaidaScreen = ({ navigation, route, isDarkMode }) => {
  ...
  const colors = useMemo(() => {
    const base = CORESCONFERENCIASAIDA;
  ...
  const persistConference = async () => {
    ...
    try {
      const raw = await AsyncStorage.getItem(STORAGE_KEYS.CONFERENCIA_SAIDAS);
      const list = raw ? JSON.parse(raw) : [];
      await AsyncStorage.setItem(
        STORAGE_KEYS.CONFERENCIA_SAIDAS,
        JSON.stringify([payload, ...list].slice(0, 300)),
      );
      ...
      if (divergences.length) {
        ...
        await AsyncStorage.setItem(
          STORAGE_KEYS.CONFERENCIA_DIVERGENCIAS,
          JSON.stringify([...divergences, ...divList].slice(0, 500)),
        );
      }
