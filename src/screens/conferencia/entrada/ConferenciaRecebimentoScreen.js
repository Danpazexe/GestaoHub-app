...
import { CORESCONFERENCIARECB } from '../../../components/coresAuth';
import { STORAGE_KEYS } from '../../../constants/storage';
import { computeTotals, normalizeKey, pluralize } from '../shared/conferenciaCore';
import { buildExpectedItemsEntrada } from '../shared/conferenciaMock';
import { useConferenciaDrafts } from '../shared/hooks/useConferenciaDrafts';
import { buildBonusRecebimentoList } from './conferenciaBonusMock';
...
const ConferenciaRecebimentoScreen = ({ navigation, route, isDarkMode }) => {
  ...
  const colors = useMemo(() => {
    const base = CORESCONFERENCIARECB;
  ...
  const draftApi = useConferenciaDrafts({
  ...
  const persistConference = async () => {
    ...
    try {
      const raw = await AsyncStorage.getItem(STORAGE_KEYS.CONFERENCIA_RECEBIMENTOS);
      const list = raw ? JSON.parse(raw) : [];
      await AsyncStorage.setItem(STORAGE_KEYS.CONFERENCIA_RECEBIMENTOS, JSON.stringify([payload, ...list].slice(0, 300)));
      ...
      if (divergences.length) {
        const rawDiv = await AsyncStorage.getItem(STORAGE_KEYS.CONFERENCIA_DIVERGENCIAS);
        const divList = rawDiv ? JSON.parse(rawDiv) : [];
        await AsyncStorage.setItem(STORAGE_KEYS.CONFERENCIA_DIVERGENCIAS, JSON.stringify([...divergences, ...divList].slice(0, 500)));
      }
